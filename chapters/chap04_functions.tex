...
Variables defined outside functions are ``global'' and can be read from anywhere:

\begin{lstlisting}
multiplier = 2  # global variable

def double(x):
	global multiplier # we do still need tell our function it should use the multiplyer variable from global when we mention multiplyer
    return x * multiplier  # can read global variable

print(double(5))  # prints 10
\end{lstlisting}

\begin{warning}
Modifying global variables from inside functions requires the \py{global} keyword and is generally discouraged. It makes code harder to understand and debug. Instead, pass values as parameters and return results.
\begin{lstlisting}
