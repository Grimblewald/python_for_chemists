...
\begin{tip}
Taking the habit of explicitly writing \py{return} or \py{return None} at the end of such functions can help show future readers that the lack of return value is deliberate and, and becomes necessary if the ending is different depending on one or more conditions.
\end{tip}
...
\begin{warning}
Docstrings are meant to be understood by anyone running the code and willing to use the function. Therefore, its goal is not to describe exactly what happens in the function but rather to say what it returns (A list ? A number ? Which one ? maybe nothing ?) and what its parameters are.
\end{warning}
...
from math import log10 # loads the logarithm function from the math library. 
# If we wanted to use more functions from this library, it would 
# be faster to write from math import * Here, * means everything. 
# But it would be even faster to write import math
...
\begin{tip}
\textbf{Ready to Upgrade?}
Now that you are writing more complex scripts with functions and variable scope, you might find a simple text editor limiting. If you skipped Section~\ref{subsec:ide} (IDEs) in the introduction, you are now ready to fully benefit from those tools. Go check it out!
\end{tip}
