...
\begin{enumerate}
    ...
    \item Click the big yellow ``Download Python'' button (get version 3.11 or later)
...
\begin{enumerate}
    ...
    \item You should see something like \shell{Python 3.11.4}
\end{enumerate}
...
\begin{enumerate}
    \item Go to \url{https://www.python.org/downloads/}
    \item Download the macOS installer (version 3.11 or later)
...
If Python 3.9 or later is installed, you're done. If not, or if you need a newer version:
...
Each operating system has its own terminal program:
...
You'll also hear about Anaconda Prompt. Anaconda is an alternative Python distribution popular in data science. It's fine, but adds complexity. We'll use standard Python to keep things simple.
...
Python responds immediately with the answer. Now try something more interesting:
...
This section might seem bureaucratic, but it solves real problems. Pay attention.

\subsection{The Problem}

Python by itself can do basic things, but its real power comes from \emph{packages} (also called \emph{libraries})---collections of code that other people have written and shared. Think of them like apps for your phone: Python is the operating system, and packages add new abilities. NumPy does numerical computing, Matplotlib makes plots, Pandas handles data tables. You'll install many of them.

But here's the problem: different projects might need different versions of the same package. Project A needs NumPy 1.24; Project B needs NumPy 1.21 because it works with an older package that doesn't support the new version.

If you install packages globally (available everywhere on your computer), you can only have one version. Updating for Project A breaks Project B. This is called ``dependency hell,'' and it's why scientists often say ``I reinstalled Python and now nothing works.''

\subsection{The Solution}

Virtual environments. Each project gets its own isolated Python environment with its own packages. Project A's environment has NumPy 1.24. Project B's environment has NumPy 1.21. They don't interfere with each other.

Think of it like having separate lab benches for different experiments. You wouldn't mix reagents from two different projects on the same bench.
...
\begin{lstlisting}[style=shellstyle]
cd Documents/python_projects/learning
\end{lstlisting}
...
This creates a \filepath{venv} folder containing a private Python installation.
...
result = 6 * 7
print(f"Six times seven is {result}")
\end{lstlisting}
...
\begin{tip}
I recommend starting with a basic text editor for your first few scripts. This builds understanding of what Python files really are. You won't develop false beliefs like ``I need special software for this script to work.'' Once you're comfortable with the basics, you can upgrade to a more powerful tool.
\end{tip}

\subsection{Upgrading Later: Integrated Development Environments}

An IDE (Integrated Development Environment) is an editor designed specifically for programming. It provides helpful features: syntax highlighting, automatic error detection, tools to explore variables while your code runs, and more. When you're ready to upgrade from a basic text editor, here are the options.

\textbf{Spyder:} Designed specifically for scientific Python. If you've used RStudio, Spyder will feel familiar---it has a similar layout with your code on one side and a variable explorer on the other. It shows you the values of all your variables as your code runs, which is incredibly helpful for learning and for data analysis.
...
Or download it from \url{https://www.spyder-ide.org/}.

\begin{tip}
For scientists, I recommend Spyder when you're ready for an IDE. Its variable explorer lets you see your data as you work, which matches how scientists think about analysis. The learning curve is gentler than other options, and it's built specifically for the kind of work you'll be doing.
\end{tip}

\textbf{Visual Studio Code (VS Code):} Made by Microsoft, free, and extremely popular. It's a general-purpose programming editor used for many languages. Download from \url{https://code.visualstudio.com/}. After installing, you'll need to add the Python extension: click the Extensions icon (four squares) in the sidebar, search for ``Python,'' and install it.

\begin{warning}
VS Code is powerful but designed for professional software developers. It has many settings and features that can confuse beginners. For example, it might silently change which Python installation it uses, breaking your code in mysterious ways. The popularity of VS Code means you'll find lots of online tutorials using it, so it's worth knowing about. But for scientific work, Spyder is usually a better fit.
\end{warning}
